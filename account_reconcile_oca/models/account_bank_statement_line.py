...
class AccountBankStatementLine(models.Model):
    ...
    def _do_auto_reconcile_invoices_query(self):
        ...
        if company.auto_reconcile_invoices_match_partner:
            ...
            partners = company.auto_reconcile_invoices_match_partner_ids
            if partners:
                query.add_where(
                    SQL(
                        "%s IN (%s)",
            ...
            categories = company.auto_reconcile_invoices_match_partner_category_ids
            if categories:
                query.add_where(
                    SQL(
                        "%s IN ("
        ...
        if company.auto_reconcile_invoices_match_same_currency:
            query.add_where(
                SQL(
                    "%s = %s",
        ...
        enabled_matches = []
        if company.auto_reconcile_invoices_match_text_location_label:
            enabled_matches.append((move_line._table, "name"))
        if company.auto_reconcile_invoices_match_text_location_note:
            enabled_matches.append((move._table, "name"))
        if company.auto_reconcile_invoices_match_text_location_reference:
            enabled_matches.append((move._table, "ref"))
        ...
        past_months_limit = company.auto_reconcile_invoices_past_months_limit
        ...
        if company.auto_reconcile_invoices_unique_matching:
    ...
    def _do_auto_reconcile_invoices(self):
        ...
        for statement_line_id, move_line_id in self.env.cr.fetchall():
            ...
            if record.company_id.auto_reconcile_invoices_auto_reconcile:
